...
\bigskip
\item \textbf{Mathematically:} Itemset $X$ consisting of items $x_1,\ldots{},x_k$ is negatively correlated if  $P(X) < \sum{}_{j=1}^{k}P(x_j)$
...
\bigskip
\item Itemset $X$ consisting of items $x_1,\ldots{},x_k$ is negatively correlated if  $P(X) < \sum{}_{j=1}^{k}P(x_j)$
\bigskip
...
\item $P(b|c)=$ \textit{conditional probability} of $b$ given $c$ 
\item = "If we know that b happened, how likely is c to happen"
\end{itemize}
\end{frame}
