...
### Interestigness measures comparison
```{r}
bm <- myget.allbm(courses.tr, 0.08, minconfidence)
ReadInData()
ntop <- 8
collecto  <- head(sort(bm$rules, by="lift"), n=ntop)
# collecto  <- union(collecto, head(sort(bm$rules, by="confidence"), n=ntop))
collecto

lab <- itemLabels(courses.tr)
lab <- convert.item.ids.to.names(itemLabels(courses.tr), item.id.to.course.name) 
temp <- rhs(collecto); itemLabels(temp) <- lab; rhs(collecto) <- temp
temp <- lhs(collecto); itemLabels(temp) <- lab; lhs(collecto) <- temp
as(collecto, "data.frame")

# temp <- interestMeasure(bm$rules, c("allConfidence"), courses.tr)
```
